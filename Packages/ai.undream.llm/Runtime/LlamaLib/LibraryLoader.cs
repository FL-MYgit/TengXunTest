...
namespace UndreamAI.LlamaLib
{
    ...
    static class LibraryLoader
    {
        ...
        public static IntPtr LoadLibrary(string libraryPath)
        {
            ...
            {
                // Use the Unicode Windows API so native libraries can be loaded
                // when the Unity project path contains non-ASCII characters.
                IntPtr handle = Win32.LoadLibrary(libraryPath);
                if (handle == IntPtr.Zero)
                {
                    int error = Marshal.GetLastWin32Error();
                    throw new DllNotFoundException(
                        $"Unable to load native library '{libraryPath}' (Windows error {error}).");
                }
                return handle;
            }
        ...
        private static class Win32
        {
            ...
            [DllImport(SystemLibrary, EntryPoint = "LoadLibraryW", SetLastError = true,
