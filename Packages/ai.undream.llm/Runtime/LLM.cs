...
namespace LLMUnity
{
    ...
    public class LLM : MonoBehaviour
    {
        ...
        {
            // Native llama.cpp diagnostics are extremely verbose. Only attach
            // them when the package is explicitly set to Debug mode.
            if (LLMUnitySetup.DebugMode == LLMUnitySetup.DebugModeType.Debug)
            {
                int debugLevel = 5;
        ...
        public static string GetLLMManagerAssetRuntime(string path)
        {
            ...
            // Preserve an explicitly assigned absolute path. This is important
            // on Windows when an ASCII-only junction is used for a model whose
            // original cache path contains characters unsupported by the
            // native library's narrow-string API.
